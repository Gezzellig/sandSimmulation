...
Grid *relax_grid(Grid *grid, double mu, double move_factor)
{
    ...
    Grid *relaxed_grid = new Grid(lambda_val);
...
Grid *relax_grid_n_times(Grid *grid, size_t n, double mu, double move_factor)
{
    ...
    {
        Grid *new_grid = relax_grid(grid, mu, move_factor);
//        delete grid;
        grid = new_grid;
    }
    return grid;
...
double calc_strain(Spring spring, double lambda_val)
{
    ...
    return magnitude / lambda_val - 1.0;
}
...
Spring *max_strain_spring(Grid *grid)
{
    ...
    for (Spring *spring : grid->springs)
    {
        double rel_strain = calc_strain(*spring, grid->lambda_val) - spring->strain_threshold;
...
list<Grid*> decrease_lambda_loop(Grid *grid, double min_lambda, double decrement_step_size, size_t n, double mu, double move_factor)
{
    ...
    {
        cout << "Current Lambda: " << grid->lambda_val << endl;
