...
def link_points(height, width, strain_normal, strain_deviation, points_grid):
	...
	for h in range(1, height-1):
		...
		spring = create_spring_with_check(h, 1, height, width, strain_normal, strain_deviation, point, points_grid)
		if not spring is None:
			springs.append(spring)
			
	for w in range(1, width-1):
		point = points_grid[0][w]
		spring = create_spring_with_check(1, w, height, width, strain_normal, strain_deviation, point, points_grid)
		if not spring is None:
			springs.append(spring)
	...
	for h in range(1, height-1):
		for w in range(1, width-1):
			...
			spring = create_spring_with_check(h+1, w, height, width, strain_normal, strain_deviation, point, points_grid)
			if not spring is None:
				springs.append(spring)
			spring = create_spring_with_check(h, w+1, height, width, strain_normal, strain_deviation, point, points_grid)
			if not spring is None:
				springs.append(spring)
	return springs
